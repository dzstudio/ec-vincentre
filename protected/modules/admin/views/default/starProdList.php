<div class="list-header">
  <h1><?php echo Yii::t('default', 'star_prod') ?></h1>
  <a class="action-button" href="javascript:unsetStarProducts()"><?php echo Yii::t('default', 'unset')?></a>
...
function unsetStarProducts() {
  var inputs = $('#listContent td input:checked');
  if (inputs.length > 0) {
    if (confirm('<?php echo Yii::t('default', 'confirm_unset_starprod')?>')) {
      var ids = [];
      inputs.each(function() {
        ids.push($(this).attr('objId'));
      });
      spinner.show();
      $.post('<?php echo Common::makeUrl('admin/default/unsetStarProducts') ?>', {'ids':ids}, function(response) {
        spinner.hide();
        response = JSON.parse(response);
        if (response.status == 'success') {
          window.location.reload();
